...
fun MainScreen() {
    ...
    Scaffold(
        ...
        NavHost(navController = navController, startDestination = NavScreen.Home.route, modifier = Modifier.padding(innerPadding)) {
            ...
            composable("review/{productCode}") { backStackEntry ->
                val code = backStackEntry.arguments?.getString("productCode") ?: ""
                ReviewScreen(navController, code)
            }
            composable("editProfile") { EditProfileScreen(navController) }
            composable("address") { AddressScreen(navController) }
            composable("address/{addressId}") { backStackEntry ->
                val addressId = backStackEntry.arguments?.getString("addressId")
                AddressScreen(navController, addressId)
            }
